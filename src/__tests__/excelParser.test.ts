...
describe('matchHeaders', () => {
  it('matches exact Korean headers', () => {
    ...
    expect(result.newColumns).toEqual(['부서'])
  })
  ...
  it('matches headers with extra whitespace', () => {
    ...
    expect(result.newColumns).toHaveLength(0)
  })

  it('returns unmatched when field labels missing from excel', () => {
    const result = matchHeaders(['foo', 'bar'], ['이름', '소속'])
    expect(result.matched).toHaveLength(0)
    expect(result.unmatched).toEqual(['이름', '소속'])
    expect(result.newColumns).toEqual(['foo', 'bar'])
  })
})
