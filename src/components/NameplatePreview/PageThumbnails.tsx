...
function ThumbnailFace({
  ...
  const scale = THUMB_WIDTH / widthPx
  ...
  return (
    <div style={{ width: THUMB_WIDTH, height: thumbH, position: 'relative', overflow: 'hidden', ...bgStyle }}>
    ...
    <div className="w-full mt-5">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'thin' }}>
        {visible.map((row, i) => (
          ...
          >
            <ThumbnailFace row={row} fields={fields} size={size} backgroundImage={backgroundImage} />
            <div
              className="text-center bg-white border-t border-gray-100 text-gray-500"
              style={{ width: THUMB_WIDTH, fontSize: 9, padding: '2px 4px', lineHeight: 1.5, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}
            >
        ...
        {overflow > 0 && (
          <div className="flex-shrink-0 flex items-center px-3 text-xs text-gray-400">
            +{overflow}명
          </div>
        )}
      </div>
