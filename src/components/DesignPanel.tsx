...
export function DesignPanel({ open, onOpenChange, state, onApply, showHint }: Props) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setEnabled(true)
      if (res.status === 401) return // 로그인 전
      if (!res.ok) {
      ...
      >
        {showHint && (
          <span className="text-[11px] font-medium text-gray-700 whitespace-nowrap">
            자주 쓰는 배경·오버레이 디자인을 저장해 두고 불러오세요
          </span>
        )}
        <button
      ...
      >
        <div className="p-4 pb-8">
          <div className="flex items-center gap-2 mb-4">
            <ImageIcon className="w-4 h-4 text-[#475569] shrink-0" />
            <h2 className="text-sm font-bold text-gray-800">내 디자인</h2>
          </div>

          {!email ? (
            <p className="text-xs text-gray-500 text-center py-10 leading-relaxed">
              구글로 로그인하면 배경·오버레이 디자인을<br />
            ...
            <>
              ...
              <div className="px-3 pt-3 pb-2 rounded border border-gray-200 bg-gray-50 mb-4">
                ...
                <p className="text-[11px] text-gray-400 mt-2 leading-relaxed">
                  이미지는 본인 구글 드라이브의 &ldquo;명패 제작기&rdquo; 폴더에 저장되고, 배치값과 텍스트
                  서식은 디자인 시트에 저장됩니다.
                </p>
              </div>
