...
type Props = {
  count: number
  /** 선택 항목 중 텍스트 항목 수 (색상 변경 가능 여부 판단) */
...
export function MultiSelectToolbar({
  ...
  onReferenceChange, onAlign, onColorChange, onClear,
}: Props) {
  return (
    <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 rounded-md border border-gray-300 bg-white/95 px-2.5 py-1.5 shadow-lg backdrop-blur">
      ...
      <label
        className={`flex shrink-0 items-center gap-1 text-[10px] ${
          textFieldCount === 0 ? 'opacity-40' : 'text-gray-500'
        }`}
        title={
          textFieldCount === 0
            ? '선택 항목에 텍스트가 없습니다'
